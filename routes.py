...
def submit_response():
    ...
    # Duplicate prevention: check if this user already submitted this prompt
    existing = responses_collection.find_one({
        "userId": ObjectId(user_id),
        "prompt": data["prompt"]
    })
    if existing:
        return jsonify({"error": "You have already submitted a response for this prompt."}), 409
    ...
    try:
        ...
        return jsonify({"error": "Failed to save response. Please try again later."}), 500
